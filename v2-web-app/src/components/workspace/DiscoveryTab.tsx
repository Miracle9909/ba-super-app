...
export const DiscoveryTab: React.FC<DiscoveryTabProps> = ({ projectId }) => {
  ...
  const [extractedData, setExtractedData] = useState<{entities: string[], rules: string[], terms: string[]}>({ entities: [], rules: [], terms: [] });
  ...
  useEffect(() => {
    if (extractedKnowledge) {
      const entities = extractedKnowledge.filter(k => k.type === 'entity').map(k => k.content);
      const rules = extractedKnowledge.filter(k => k.type === 'rule').map(k => k.content);
      const terms = extractedKnowledge.filter(k => k.type === 'term').map(k => k.content);
      setExtractedData({
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-5" style={{ gridTemplateColumns: '1.5fr 1fr' }}>
        ...
        <Card className="flex flex-col" style={{ minHeight: 420 }}>
          <div className="px-5 py-3 border-b border-border-color flex items-center gap-2">
          ...
          <div className="space-y-4">
              ...
              {extractedData.entities.length > 0 ? (
                <div className="flex flex-wrap gap-1.5">
                  {extractedData.entities.map((e, i) => (
                    <span key={i} className="text-[12px] px-2.5 py-1 rounded-lg border border-border-color bg-white text-text-primary font-medium">
                      {e.split('[')[0].trim().substring(0, 40)}
                    </span>
                  ))}
                </div>
              ) : (
            ...
            <div className="rounded-xl p-4 border border-border-color" style={{ background: '#fcfdff' }}>
              ...
              {extractedData.rules.length > 0 ? (
                <div className="space-y-1.5">
                  {extractedData.rules.map((r, i) => (
                    <div key={i} className="text-[12.5px] text-text-primary pl-3 border-l-2" style={{ borderColor: '#fbbf24' }}>
                      {r.substring(0, 80)}
                    </div>
                  ))}
                </div>
            ...
            <div className="rounded-xl p-4 border border-border-color" style={{ background: '#fcfdff' }}>
              ...
              {extractedData.terms.length > 0 ? (
                <div className="space-y-1.5">
                  {extractedData.terms.map((t, i) => (
                    <div key={i} className="text-[12.5px] text-text-primary pl-3 border-l-2 border-green-300">
                      {t.substring(0, 80)}
                    </div>
                  ))}
                </div>
      ...
      {selectedOutputs.length > 0 && (
        ...
        >
          ...
          <PrimaryBtn icon="sparkles" onClick={() => console.log('generate', selectedOutputs)}>
